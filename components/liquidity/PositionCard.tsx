    ...
    <div className="border border-line bg-panel p-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col items-end gap-1">
          {isLocked ? (
            <span className="flex items-center gap-1 border border-gold/50 bg-gold/10 px-2 py-1 font-mono text-[0.5625rem] uppercase tracking-wider2 text-goldLight">
              <LockIcon className="h-3 w-3" />
              Locked
            </span>
          ) : (
            <span className="border border-emeraldLight/50 bg-emerald/30 px-2 py-1 font-mono text-[0.5625rem] uppercase tracking-wider2 text-emeraldLight">
              Unlocked
            </span>
          )}
          {isBoosted && (
      ...
      <div className="mt-5 flex items-center justify-between">
        <span className="font-mono text-[0.625rem] uppercase tracking-wider2 text-bronze">Your Position</span>
        <span className="font-display text-lg text-ivory">
          {position.valueUsd !== null ? formatUsd(position.valueUsd) : "—"}
        </span>
      </div>
      <div className="mt-2 flex items-center justify-between">
        <span className="font-mono text-[0.625rem] uppercase tracking-wider2 text-bronze">Pool Share</span>
      ...
      <button
        type="button"
        onClick={onManage}
        className="mt-5 w-full border border-gold px-4 py-2 font-mono text-[0.625rem] uppercase tracking-wider2 text-goldLight transition-colors hover:bg-gold hover:text-void"
      >
        Manage
      </button>
    </div>
