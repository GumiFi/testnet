import { RocketIcon, CrownIcon } from "@/components/icons";
import Avatar from "@/components/discover/Avatar";
import { poolPairLabel, type LiquidityPool } from "@/lib/liquidity-data";
import { formatCompactUsd } from "@/lib/format";

function PoolPairLabel({ pool }: { pool: LiquidityPool }) {
  return (
  ...
  if (variant === "row") {
    return (
      ...
      >
        ...
        <span className="text-right font-mono text-xs text-ivory">{formatCompactUsd(pool.tvlUsd)}</span>
        <span className="text-right font-mono text-xs text-ivory">
          {formatCompactUsd(pool.volume24hUsd)}
        </span>
        <span className="text-right font-mono text-xs text-goldLight">{pool.aprPct.toFixed(1)}%</span>
      </button>
    );
  }
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-line pt-3">
        <div>
          ...
          <p className="mt-1 font-mono text-xs text-ivory">{formatCompactUsd(pool.tvlUsd)}</p>
        </div>
        <div>
          <p className="font-mono text-[9px] uppercase tracking-wider2 text-bronze">Volume 24H</p>
          <p className="mt-1 font-mono text-xs text-ivory">{formatCompactUsd(pool.volume24hUsd)}</p>
        </div>
        <div>
          <p className="font-mono text-[9px] uppercase tracking-wider2 text-bronze">APR</p>
          <p className="mt-1 font-mono text-xs text-goldLight">{pool.aprPct.toFixed(1)}%</p>
        </div>
      </div>
    </button>
  );
